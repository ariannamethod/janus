/*
 * janus4-bpe.c — Janus Pure BPE Post-Transformer (4-attention)
 *
 * Pure BPE version: BPE in, BPE out. No char-level pressure.
 * Hybrid attention (QKV + RRPRAM + Janus self-resonance + Temporal Delta).
 * Calendar Drift, AML physics, Chuck optimizer, dual weight matrices.
 * 12 bi-directional associative reasoning steps.
 *
 *   cc janus4-bpe.c -O2 -lm -o janus4-bpe
 *   ./janus4-bpe --train data.txt --steps 5000
 *   ./janus4-bpe --generate "To be or not"
 *
...
typedef struct {
    ...
    float *wr[MAX_BLK],*wvr[MAX_BLK],*wj[MAX_BLK],*wd[MAX_BLK],*gate[MAX_BLK];
...
static int model_size(void){
    ...
    for(int b=0;b<BLOCKS;b++){
        ...
        s+=HEADS*DIM*HEAD_DIM; s+=DIM*DIM; s+=DIM*DIM; s+=HEADS*4;
...
static void assign_ptrs(Ptrs*p,float*q){
    ...
    for(int b=0;b<BLOCKS;b++){
        ...
        p->wd[b]=q;q+=DIM*DIM; p->gate[b]=q;q+=HEADS*4; p->wo[b]=q;q+=DIM*DIM;
        p->rms2[b]=q;q+=DIM;
...
static void model_init(Model*m){
    ...
    for(int b=0;b<BLOCKS;b++) for(int i=0;i<HEADS*4;i++) m->w.gate[b][i]=0;
    printf("[janus4-bpe] model: %d params (%.2fMB)\n",m->n_params,m->n_params*4.0f/1e6f);
}
...
float*q,*k,*v,*at,*ho,*ra,*rv,*ro,*je,*ja,*jo,*de,*da,*doa,*frm,*lg;}Acts;
...
static void acts_alloc(Acts*a){
    ...
    a->de=calloc(MAX_T*DIM,4);a->da=calloc(MAX_T*MAX_T,4);a->doa=calloc(MAX_T*HEAD_DIM,4);
...
static void acts_free(Acts*a){
    ...
    free(a->de);free(a->da);free(a->doa);
...
static void temporal_delta_attention(const float*x,const float*wd,float*delta,float*attn,int T){
    float prev[DIM]; memset(prev,0,sizeof(prev));
    float energy[MAX_T];
    for(int t=0;t<T;t++){
        const float *xt=x+t*DIM; float *dt=delta+t*DIM;
        float e=0;
        for(int i=0;i<DIM;i++){
            float grad=(t==0)?0.0f:(xt[i]-prev[i]);
            float s=0; for(int j=0;j<DIM;j++) s+=wd[i*DIM+j]*((j==i)?grad:0.0f);
            dt[i]=0.7f*grad+0.3f*s;
            e+=fabsf(dt[i]);
            prev[i]=xt[i];
        }
        energy[t]=e/(DIM+1e-6f);
    }
    for(int i=0;i<T;i++){
        for(int j=0;j<T;j++){
            if(j>i){attn[i*T+j]=-1e9f;continue;}
            float dist=(float)(i-j);
            float decay=1.0f/(1.0f+dist);
            attn[i*T+j]=(energy[i]*energy[j])*decay;
        }
...
static float forward(Ptrs*w,Acts*a,int*tok,int*tgt,int T){
    ...
    for(int b=0;b<BLOCKS;b++){
        ...
        for(int h=0;h<HEADS;h++){
            ...
            if(h==0) temporal_delta_attention(a->rm1,w->wd[b],a->de,a->da,T);
            for(int t=0;t<T;t++) for(int d=0;d<HEAD_DIM;d++){
                float s=0;for(int j=0;j<=t;j++)s+=a->ja[t*T+j]*a->je[j*DIM+h*HEAD_DIM+d];
                a->jo[t*HEAD_DIM+d]=s;}
            for(int t=0;t<T;t++) for(int d=0;d<HEAD_DIM;d++){
                float s=0;for(int j=0;j<=t;j++)s+=a->da[t*T+j]*a->de[j*DIM+h*HEAD_DIM+d];
                a->doa[t*HEAD_DIM+d]=s;}
            float gl[4]={w->gate[b][h*4],w->gate[b][h*4+1],w->gate[b][h*4+2],w->gate[b][h*4+3]};row_softmax(gl,4);
            for(int t=0;t<T;t++) for(int d=0;d<HEAD_DIM;d++)
                a->cat[t*DIM+h*HEAD_DIM+d]=gl[0]*a->ho[t*HEAD_DIM+d]+gl[1]*a->ro[t*HEAD_DIM+d]+gl[2]*a->jo[t*HEAD_DIM+d]+gl[3]*a->doa[t*HEAD_DIM+d];
        }
...
static void train_bpe(DualModel*dm,const char*path,int max_steps,float lr){
    ...
    printf("[janus4-bpe] learning %d BPE merges...\n",nm);
    bpe_learn_merges(raw,fsz,nm);
    printf("[janus4-bpe] BPE vocab: %d\n",BPE.vocab_size);
    ...
    printf("[janus4-bpe] %ld bytes → %d tokens\n",fsz,bl);
    printf("[janus4-bpe] params: %d (%.2fMB) × 2 matrices\n",dm->A.n_params,dm->A.n_params*4.0f/1e6f);
    printf("[janus4-bpe] Chuck optimizer, %d steps, lr=%.1e, grad_accum=%d\n",max_steps,lr,GRAD_ACCUM);
    ...
    printf("[janus4-bpe] done. best=%.4f\n",best);
...
static void save_model(DualModel*dm,const char*p){
    ...
    fclose(f);printf("[janus4-bpe] saved to %s\n",p);
}
...
static int load_model(DualModel*dm,const char*p){
    ...
    fclose(f);printf("[janus4-bpe] loaded from %s\n",p);return 0;
}
...
int main(int argc,char**argv){
    ...
    if(!tp&&!pr){printf("\n[janus4-bpe] interactive mode\n");char buf[1024];
    while(1){printf("\njanus4-bpe> ");if(!fgets(buf,sizeof(buf),stdin))break;
